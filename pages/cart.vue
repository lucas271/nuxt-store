<template>
    <v-main class="full-height-minus-navheader">
            ...
            <v-card class="h-100 w-100 d-flex flex-column justify-space-between pa-3 bg-teal-lighten-4 ">
                ...
                <div class="d-flex flex-column ga-4 overflow-auto my-3 w-100 flex-grow-1 flex-shrink-1" >
                    <template v-if="!cartStore.loading">
                        <div class="h-75 w-100" v-for='product in cartStore.cartProducts'>
                            <div class="d-flex w-100 h-50 text-grey-darken-4 justify-space-evenly py-2 px-1 flex-column flex-md-row align-center ga-4 flex-grow-1 flex-shrink-1 overflow-auto">
                            ...
                            <div class="d-flex  flex-sm-row flex-column ga-2 align-center justify-space-between w-100">
                                <div class="d-flex w-100 ga-2 justify-space-between align-center">
                                    <v-btn :active='product.product?.loading === false' density="compact" variant='tonal' icon="mdi-arrow-left" @click='cartStore.deleteSingleCartProduct(item.product.id)'/>
                                    <span class="text-body-2" v-if='!product.product.loading'>{{product.quantity}}</span>
                                    <span> <v-progress-circular indeterminate/> </span>
                                    <v-btn :active='product.product?.loading === false' density="compact" variant='tonal' icon="mdi-arrow-right" @click='cartStore.addProduct(item.product.id)' />
                                </div>
                ...
                <footer class="d-flex w-100 justify-space-between align-center">
                    ...
                    <span class='font-weight-bold text-uppercase'>Total: R${{cartStore.cartProducts.reduce((pv, cv) => ({totalPrice: (cv.quantity * cv.product.price) + pv.totalPrice}), {totalPrice: 0, quantity: 0}).totalPrice.toFixed(2).replace('.', ',')}}</span>
                </footer>
            </v-card>
        </v-container>
    </v-main>
