<template>
    <v-card class="bg-teal-lighten-5 pa-2 rounded-md text-blue-grey-darken-4 d-flex flex-column"       width="50vw" height="45vh">
        <header>
            <v-card-title tag="h5" class="d-flex justify-start ga-2 items-center "><v-icon>mdi-cart-outline</v-icon> Carrinho</v-card-title>
        ...
        <template v-if='!cartStore.loading'>
            <div class="d-flex flex-column ga-4 flex-shrink-1 flex-grow-1  my-3 overflow-y-auto overflow-x-hidden px-sm-4">
                <div class="d-flex w-100 h-75 overflow-hidden flex-shrink-0 ga-2 justify-space-between" :style="{position: 'relative'}" v-if='cartStore?.cartProducts?.length > 0' v-for='item in cartStore.cartProducts'>
                    ...
                    <div class="pa-2 d-flex py-1  px-1 h-100 overflow-auto justify-center justify-lg-space-evenly align-center flex-shrink-1 flex-grow-1 ga-4 w-50  my-auto">
                        <article class="d-flex ga-1 flex-column  align-lg-center align-md-start h-auto flex-shrink-1 justify-lg-center overflow-auto flex-grow-1">
                            <v-card-title tag="h6" class="text-md-h6 text-sm-subtitle-1 ma-0 pa-0 text-blue-grey-darken-3 text-wrap flex-shrink-1 flex-grow-1 flex-lg-grow-0">{{item.product.name}}</v-card-title>
                            <v-card-subtitle tag="p" class="text-break text-wrap ma-0 pa-0 text-caption text-lg-center text-blue-grey-darken-5 font-bold">UN: R${{Number(item.product.price).toFixed(2).replace('.', ',')}}</v-card-subtitle>
                        </article>
                ...
                <div  class="d-flex w-100 h-75 overflow-hidden flex-shrink-0 justify-space-evenly align-center flex-column" v-else>
                    <span class='text-center'>
                        <span class="text-h4"> Seu carrinho está vazio... </span>
                        <span class="text-subtitle-2"> Clique no botão para procurar items </span>
                    </span>
            ...
            <v-card-text tag="footer" class="w-100 pt-2 text-subtitle-2 d-flex justify-space-between align-center">
