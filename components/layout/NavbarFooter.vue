<template>
    <div class="w-100">
        <v-bottom-navigation class="d-sm-none w-100 d-flex justify-center items-center ga-3 bg-teal-lighten-5 text-teal-lighten-3" height="55" fixed shift tag="footer">
            ...
            <v-btn value="favorites" :aria-label="!data?.session ? 'ir para pagina de autentificação' : 'sair'" class="h-100" @click="navigateTo('auth')">
                <v-icon v-if="!data?.session">mdi-account-outline</v-icon>
                <v-icon v-else>mdi-exit-run</v-icon>
            </v-btn>

            <v-btn value="nearby" aria-label="ir para favoritos" class="h-100" v-if="data?.session" @click="navigateTo('wishList')">
