<template>
    ...
    :style="{position: 'absolute', zIndex: 70, [x === 'left' ? 'left' : 'right']: '2.5%', [y === 'top' ? 'top': 'bottom']: '2.5%'}">
        <v-hover>
            <template v-slot:default="{ isHovering, props }">
                <v-btn v-bind='props' :disabled="wishListStore.loading" @click="wishListStore.wishList.find(item => item.product.id == (id || divRef?.parentElement?.parentElement?.attributes?.name?.value)) ? wishListStore.removeFromWishList(id || divRef?.parentElement?.parentElement?.attributes?.name?.value) : wishListStore.addToWishList(id || divRef?.parentElement?.parentElement?.attributes?.name?.value)" :class="'text-subtitle-2 ' + !isHovering ? 'text-red-darken-2' : 'text-red-darken-4'" :icon="wishListStore.wishList.find(item => item.product.id == (id || divRef?.parentElement?.parentElement?.attributes?.name?.value)) ? (isHovering ? 'mdi-heart-broken': 'mdi-heart') : (isHovering ? 'mdi-heart' : 'mdi-heart-outline')" density="comfortable"></v-btn>
