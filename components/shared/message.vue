<template>
    <div class="bg-white ma-2 d-flex flex-column pa-2 d-flex rounded-lg">
        ...
        <header class="w-100 d-flex flex-column ga-2">
            <span class="d-flex justify-space-between align-center flex-wrap ga-1 text-subtitle-2">
            ...
            <v-card-subtitle tag="h5" class="ma-0 pa-0">{{userName}}</v-card-subtitle>


        </header>
        <div class="flex-grow-1 flex-shrink-1 w-100">
            <v-card-text tag="p" class="pa-0 ma-1 overflow-auto bg-teal-darken-3 pa-2 rounded-lg" :style="{minHeight: '8vh', maxHeight: '15vh'}">
                {{message}}
            </v-card-text>
        </div>
    </div>
