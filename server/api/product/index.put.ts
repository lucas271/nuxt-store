...
export default defineEventHandler(async (event) => {
  try{
    ...
    const product = new Product({...body, userId})
		...
		if(body.type === 'updateProduct') return await defaultResponse(product, product.updateProduct.bind(product), 'message')
		else if(body.type === 'increaseProductQuant') return await defaultResponse(product, product.increaseProductQuant.bind(product), 'message')
		else if(body.type === 'decreaseProductQuantByOne') return await defaultResponse(product, product.decreaseProductQuantByOne.bind(product), 'message')
  }
